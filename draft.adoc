...
crecimiento de usuaros se ha disparado en los últimos años (Ver __Figura 1__). Hoy
...
RFC 6455 <%https://tools.ietf.org/html/rfc6455%> define WebSocket (Figura 3) como un protocolo que proporciona un canal
...
Java (Figura 5) es un lenguaje de programación de propósito general, concurrente y orientado
...
Vert.x<%http://vertx.io/%> (Figura 6) es un toolkit de Java que permite construir aplicaciones reactivas.
...
- *Paso 7*: TestResultServer publica el resultado en el EventBus, de forma que el cliente es capaz de recibirlo y añadirlo a su gráfica correspondiente. En la figura 19 podemos ver una captura de como se muestran estos resultados en el navegador.
...
Para nuestro caso de estudio, podemos definir la latencia o tiempo de respuesta como el retardo en milisegundos producido por la demora en la propagación de los mensajes, como podemos apreciar en la figura 20.
...
A continuación se muestran los resultados de latencia para distinto número de salas de chat (Figuras 21, 22 y 23):
...
A continuación se muestran los resultados de uso de CPU para distinto número de salas de chat (Figuras 24, 25 y 26):
...
A continuación se muestran los resultados de uso de memoria para distinto número de salas de chat (Figuras 27, 28 y 29):
...
Para obtener una visión global de la comparativa, haremos uso un gráfico de dispersión (Ver __Figura 30__) para el caso más representativo por su alta carga de trabajo, 60 usuarios en una sola sala de chat, con un total de 1,8 millones de mensajes enviados y recibidos.
